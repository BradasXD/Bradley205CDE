...
class CommentForm(Form):
    ...
    comments = TextAreaField('comment', validators=[DataRequired(), Length(min=3, max=10)])
...
def view_form():
    ...
    if form.validate_on_submit():
        ...
        with sqlite3.connect(app.config['feedback']) as con:
            cur = con.cursor()
            cur.execute("INSERT INTO comments_table (name, comment) VALUES (?,?)", (name, comments))
            con.commit()
